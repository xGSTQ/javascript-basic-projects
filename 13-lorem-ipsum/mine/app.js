...
form.addEventListener('submit', (e) => {
  ...
  e.preventDefault();
  // grab the input value (all inputs return a string)
  ...
  if (isNaN(value) || value <= 0 || value > 9) {
    ...
    const previousResults = document.querySelectorAll('p.result');
    previousResults.forEach(item => {
      item.remove();
    })

    // set tempText array to 0 to clear any previous results first
    let tempText = 0;
    // slice the text array into a new tempText array
    // https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/slice
    tempText = text.slice(0, value);

    // forEach the new array item
